...
type OpenSSLResult<T> = Result<T, openssl::error::ErrorStack>;
fn encrypt_aes_ecb(plaintext: &[u8], key: &[u8]) -> OpenSSLResult<Vec<u8>> {
    let cipher = symm::Cipher::aes_128_ecb();
    let mut encrypter = symm::Crypter::new(cipher, symm::Mode::Encrypt, key, None)?;
    encrypter.pad(false);

    let mut ciphertext = vec![0; 32]; // openssl complains if <32
...
pub fn encrypt_aes_cbc(plaintext: &[u8], key: &[u8]) -> OpenSSLResult<Vec<u8>> {
