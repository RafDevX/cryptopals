...
pub fn solve_chall03() {
    let lines = util::read_hex_lines();
    let input = lines.first().expect("No first operand provided");
    let (key, msg) = find_single_byte_key(input).expect("No single-byte key found");
    println!("Key: {key:#02x}");
    println!("{}", util::bytes_to_hex(&msg))
}

fn find_single_byte_key(c: &[u8]) -> Option<(u8, Vec<u8>)> {
    ...
    for k in u8::MIN..=u8::MAX {
        ...
        if score > top_score {
            top_candidate = Some((k, candidate));
            top_score = score;
        }
    }
...
mod tests {
    ...
    #[test]
    fn is_correct() {
        let (key, msg) = find_single_byte_key(&util::hex_to_bytes(
